package com.dat.swipe_back.fragment;
...
public class NestedScrollableHost extends FrameLayout {
    ...
    private View parentViewPager() {
        ...
        while (v != null && !(v instanceof ViewPager2) && !(v instanceof SliderPanel)) {
    ...
    private void handleInterceptTouchEvent(MotionEvent ev) {
        View vp = this.parentViewPager();
        if (vp == null)
            return;
        int orientation = ViewPager2.ORIENTATION_HORIZONTAL;
        if (vp instanceof ViewPager2)
            orientation = ((ViewPager2) vp).getOrientation();
