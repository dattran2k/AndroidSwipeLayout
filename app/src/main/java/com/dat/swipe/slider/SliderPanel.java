...
public class SliderPanel extends FrameLayout {
    ...
    private boolean isEnableScrollRight = false;
    private final ViewDragHelper.Callback leftFacebookCallback = new ViewDragHelper.Callback() {
        @Override
        public boolean tryCaptureView(View child, int pointerId) {
            boolean edgeCase = !config.isEdgeOnly() || dragHelper.isEdgeTouched(edgePosition, pointerId);
            return child.getId() == decorView.getId() && edgeCase;
        ...
        public int clampViewPositionHorizontal(View child, int left, int dx) {
            int clamp;
            if (isEnableScrollRight)
                clamp = clamp(left, -(screenWidth / 2), screenWidth);
            else
                clamp = clamp(left, 0, screenWidth);
            return clamp;
        }
